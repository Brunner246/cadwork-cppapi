...
Frame3D::Frame3D()
    ...
    , m_axisX(Vector3D::unitX())
    , m_axisY(Vector3D::unitY())
    , m_axisZ(Vector3D::unitZ())
{
}

Frame3D::Frame3D(const Point3D& origin, const Vector3D& axisX,
                 const Vector3D& axisY, const Vector3D& axisZ)
    : m_origin(origin)
...
Point3D Frame3D::localToWorldMatrix(const Point3D& localPoint) const
{
    ...
    const double wx = matrix[0] * localPoint.x + matrix[1] * localPoint.y + 
                      matrix[2] * localPoint.z + matrix[3];
    const double wy = matrix[4] * localPoint.x + matrix[5] * localPoint.y + 
                      matrix[6] * localPoint.z + matrix[7];
...
Vector3D Frame3D::worldToLocalMatrix(const Vector3D& worldVector) const
{
    ...
    const double lx = m_axisX.x * worldVector.x + m_axisX.y * worldVector.y + m_axisX.z * worldVector.z;
    const double ly = m_axisY.x * worldVector.x + m_axisY.y * worldVector.y + m_axisY.z * worldVector.z;
    const double lz = m_axisZ.x * worldVector.x + m_axisZ.y * worldVector.y + m_axisZ.z * worldVector.z;
...
Vector3D Frame3D::localToWorldMatrix(const Vector3D& localVector) const
{
    ...
    const double wx = m_axisX.x * localVector.x + m_axisY.x * localVector.y + m_axisZ.x * localVector.z;
    const double wy = m_axisX.y * localVector.x + m_axisY.y * localVector.y + m_axisZ.y * localVector.z;
    const double wz = m_axisX.z * localVector.x + m_axisY.z * localVector.y + m_axisZ.z * localVector.z;
...
Point3D Frame3D::localToWorldDotProduct(const Point3D& localPoint) const
{
    ...
    const double worldX = m_origin.x + 
                          localPoint.x * m_axisX.x + 
                          localPoint.y * m_axisY.x + 
                          localPoint.z * m_axisZ.x;
    
    const double worldY = m_origin.y + 
                          localPoint.x * m_axisX.y + 
                          localPoint.y * m_axisY.y + 
                          localPoint.z * m_axisZ.y;
    
    const double worldZ = m_origin.z + 
                          localPoint.x * m_axisX.z + 
                          localPoint.y * m_axisY.z + 
                          localPoint.z * m_axisZ.z;
...
Vector3D Frame3D::localToWorldDotProduct(const Vector3D& localVector) const
{
    ...
    const double worldX = localVector.x * m_axisX.x + 
                          localVector.y * m_axisY.x + 
                          localVector.z * m_axisZ.x;
    
    const double worldY = localVector.x * m_axisX.y + 
                          localVector.y * m_axisY.y + 
                          localVector.z * m_axisZ.y;
    
    const double worldZ = localVector.x * m_axisX.z + 
                          localVector.y * m_axisY.z + 
                          localVector.z * m_axisZ.z;
...
bool Frame3D::isOrthonormal(double tolerance) const
{
    ...
    if (std::abs(m_axisX.lengthSquared() - 1.0) > tolerance) return false;
    if (std::abs(m_axisY.lengthSquared() - 1.0) > tolerance) return false;
    if (std::abs(m_axisZ.lengthSquared() - 1.0) > tolerance) return false;
    
    // Check that axes are orthogonal (dot products are zero)
    if (std::abs(m_axisX.dot(m_axisY)) > tolerance) return false;
    if (std::abs(m_axisY.dot(m_axisZ)) > tolerance) return false;
    if (std::abs(m_axisZ.dot(m_axisX)) > tolerance) return false;
